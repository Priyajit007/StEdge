...
## Design   : prewitt_traditional
## Source   : capture.v     (top module: capture)
## Clocks   : clk_100mhz, pclk (+ generated clock(s) halfclk left unconstrained)
...
create_clock -name clk_100mhz -period $CLK_PERIOD_PS [get_ports clk_100mhz]
create_clock -name pclk -period $CLK_PERIOD_PS [get_ports pclk]
...
set NONCLK_INPUTS [remove_from_collection [all_inputs] [get_ports {clk_100mhz pclk}]]
set DELAY_OUTPUTS [remove_from_collection [all_outputs] [get_ports {halfclk}]]
set_input_delay  -max 100 -clock [get_clocks clk_100mhz] $NONCLK_INPUTS
set_output_delay -max 100 -clock [get_clocks clk_100mhz] $DELAY_OUTPUTS
